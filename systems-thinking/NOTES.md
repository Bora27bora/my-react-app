# Notes

## Book the user is reading

_Thinking in Systems: A Primer_ by Donella H. Meadows (ed. Diana Wright). Structure, for reference when sequencing lessons:

- Introduction: The System Lens
- Part One: System Structure and Behavior
  - Ch 1. The Basics (elements, interconnections, purpose; stocks & flows)
  - Ch 2. A Brief Visit to the Systems Zoo
- Part Two: Systems and Us
  - Ch 3. Why Systems Work So Well
  - Ch 4. Why Systems Surprise Us
  - Ch 5. System Traps . . . and Opportunities
- Part Three: Creating Change—in Systems and in Our Philosophy
  - Ch 6. Leverage Points—Places to Intervene in a System
  - Ch 7. Living in a World of Systems
- Appendix: Glossary, Summary of Systems Principles, Springing the System Traps, Places to Intervene, Guidelines for Living in a World of Systems

## Working notes

- Lessons should track roughly alongside the user's progress through the book — check in each session on which chapter they're currently reading.
- The book itself contains an appendix glossary and a "Summary of Systems Principles" — these are good candidates to mirror/compress into `GLOSSARY.md` and `./reference/` once the corresponding chapters are taught.
